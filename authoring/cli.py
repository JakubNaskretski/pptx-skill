...
def remove_deck(deck_stem: str, dry_run: bool) -> None:
    """Remove a deck folder, its v5 theme, and every v5 skeleton sourced
    from it. Extracted assets are kept (deduplicated by content SHA,
    reusable across decks). Dead deck references in assets/<id>.yaml
    ...
    click.echo(f"assets:     {len(asset_yamls_to_prune)} yaml(s) will "
               f"have dead source pruned; binaries kept")

    if dry_run:
        for sk in skeleton_dirs:
            click.echo(f"  would drop skeleton: {sk.name}")
...
def _render_pending_backgrounds() -> tuple[int, int]:
    ...
    for sk_dir in sorted(skeletons_root.iterdir()):
        ...
        if sk.get("overlap_decision") != "freeze_pending":
            continue
        sk_id = sk.get("id") or sk_dir.name
        try:
            ...
            if deck is None or idx is None:
                continue
            slide_pptx = (
                WORKSPACE / "decks" / deck / "slides" / f"slide_{int(idx):02d}.pptx"
            )
            slot_shape_ids = {s.get("shape_id") for s in (sk.get("slots") or [])
                              if s.get("shape_id") is not None}
            ok, msg = _render_background_png(sk_id, sk_dir, slide_pptx, slot_shape_ids)
            if ok:
                ...
                try:
                    ...
                    failed += 1
                    click.echo(
                        f"preview: rendered background for {sk_id!r} but "
                ...
                failed += 1
                click.echo(
                    f"preview: background for {sk_id!r} not rendered: {msg}",
                    err=True,
                )
        except Exception as e:
            failed += 1
            click.echo(
                f"preview: background for {sk_id!r} skipped due to error: {e}",
                err=True,
            )
    return built, failed
